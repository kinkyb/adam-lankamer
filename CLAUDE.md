...
- **Collage backdrop** (`.collab-bg`, dark `#0e0e0e` base): a CSS grid of darkened work photos behind the whole block; overlay gradient (`rgba(14,14,14,0.72→0.5→0.5→0.8)`) + per-tile `filter: … brightness(0.8)` + a text `text-shadow` keep the light text readable. Tile min-width is `minmax(180px)` on desktop, `minmax(90px)` ≤900px (mobile media query) so phones get ~4 small square columns instead of 2 huge zoomed blocks. Built by `buildCollabBackdrop()` (bottom `<script>`): shuffles the `PHOTOS` array once, then **tiles to fill COMPLETE rows**. Row height = `tileWidth × aspect`, where `aspect = 1.5` (portrait ~2:3) ≤900px, `1.0` (square) on desktop — **tile SHAPE, not size, controls crop/"zoom"**: a square tile crops a portrait photo to its centre, so phones use portrait tiles to show the mostly-portrait photos with little crop (fixes the "zoomed content in squares" complaint). `grid-auto-rows` is then set to `blockHeight / round(blockHeight / rowHeight)` so rows divide the height exactly and the bottom row is flush (never clipped, never a gap). Re-renders on `load`, `document.fonts.ready`, and window `resize`. Photos repeat only as much as needed to complete the rows.
