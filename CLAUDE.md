...
- **Collage photos**: the `PHOTOS` array is ONLY the `assets/collage/` set (43 shots, web-sized via `sips -Z 900 -s format jpeg 78`, mostly 1:1). The original full-res `assets/` studio photos were **removed** from the collage — they were 6336px (far too heavy for a mobile background) and non-square (cropped badly under the old grid). To add more: resize into `assets/collage/`, append `{ src: 'assets/collage/NAME.jpg' }`. Masonry means any aspect ratio is fine (no crop); square-ish photos tile most evenly.
